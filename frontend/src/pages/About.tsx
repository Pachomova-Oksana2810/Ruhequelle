...
export default function About() {
  ...
  const certsRef = useScrollAnimation<HTMLDivElement>();

  return (
    <section ref={pageRef} className="page about-page scroll-animate">
      ...
      <div ref={certsRef} className="certs scroll-animate section-watermark">
        <h3>Meine Zertifikate</h3>
        <div className="cert-grid">
          {certImages.map((src, i) => (
            <div key={i} className="cert">
              <img src={src} alt={`Zertifikat ${i + 1}`} loading="lazy" />
            </div>
          ))}
        </div>
      </div>
